...
class TFArgument(Argument):
    ...
    def combined_mutation(self, mutate_info) -> None:
        ...
        if info_type == "ArgType.TF_TENSOR":
            self.type = ArgType.TF_TENSOR
        ...
        elif info_type == "ArgType.KERAS_TENSOR":
            self.type = ArgType.KERAS_TENSOR
            ...
            if random.random() < 0.01:
                self.value = []  # with a probability return an empty list
        elif info_type == "ArgType.TF_OBJECT" or info_type == "ArgType.TF_VARIABLE" or info_type == "ArgType.NULL":
            pass
        elif info_type == "ArgType.TF_DTYPE":
            self.type = ArgType.TF_DTYPE
...
class TFAPI(API):
    ...
    def mutate_explore(self, enable_value=True, enable_type=True, enable_db=True, param_index=0):
        ...
        with open(file_path, "a") as file:
            ...
            para_maxV = str(arg.maxv)
            para_maxV_type = type(arg.maxv)
            para_minV = str(arg.minv)
            para_minV_type = type(arg.minv)
    ...
    def to_code_oracle(self,
        ...
        if oracle == OracleType.CRASH:
            ...
            gpu_code = self.to_diff_code(prefix=prefix, res_name=RES_GPU_KEY,
                use_try=True, err_name=ERR_GPU_KEY, wrap_device=True, device_name="GPU:1")
